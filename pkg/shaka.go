...
// ShakaParser Wrapper around the fmt.Sprinter with Validate method
...
// StreamDescriptorFn Represents a function that can add a new option to a given StremaOption
// Ex:
//
...
// ShakaFlagFn Represents a function that can add new flags to *ShakaFlags
// Ex:
//
...
// ShakaPackager Responsible for calling Shaka-packager.
...
func (s *ShakaPackager) BuildAndValidate() ([]string, error) {
    var args []string
    var validationErrors []string

    args, argsError := buildStreamDescriptors(s.StreamOptions...)
    flags, flagError := buildFlags(s.Flags)
...
func (s *ShakaPackager) PreviewCommand() (string, error) {
    args, err := s.BuildAndValidate()
    if err != nil {
        return "", err
    }

    return fmt.Sprintf("%s %s", s.Binary, strings.Join(args, " ")), nil
}
